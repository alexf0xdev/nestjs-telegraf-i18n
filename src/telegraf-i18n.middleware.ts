...
export class TelegrafI18nMiddlewareProvider<K = Record<string, unknown>> {
    ...
    private static INVALID_CONTEXT_WARNING = 'The provided context is not an instance of TelegrafI18nContext. ' +
    ...
    async telegrafI18nMiddleware (ctx: Context, next: () => Promise<void>) {
        const language: string = ctx?.from?.language_code || (this.i18nService as any).getFallbackLanguage();
